...
// אובייקט JSON המכיל רשימה של נתונים המייצגים את פרטי הכרטיסים (כותרת, תמונה, ותוכן)
    ...
    let isClearing = false; // משתנה בוליאני לבדיקת מצב הכפתור: "לחיפוש" או "ניקוי"

    search.addEventListener("click", function (event) {
        event.preventDefault(); // מונע טעינה מחדש של הדף

        if (isClearing) { // בדיקה אם הכפתור במצב "ניקוי"
            searchInput.value = ""; // מנקה את שדה החיפוש
            createAllCards(); // מציג מחדש את כל הכרטיסים
            search.innerHTML = "לחיפוש"; // מחזיר את הטקסט של הכפתור למצבו המקורי
            isClearing = false; // משנה את המצב של הכפתור חזרה ל"לחיפוש"
            return; // מפסיק את המשך הקוד
        }

        const searchValue = searchInput.value.trim(); // מקבל את הערך מהשדה ומסיר רווחים מיותרים
        ...
        // חיפוש הכרטיסיות המתאימות לפי הכותרת
        const results = jsonData.generators.filter(generator => generator.title.includes(searchValue));
        itemContainer.innerHTML = ""; // מנקה את התוכן הקיים

        if (results.length > 0) { // אם נמצאו תוצאות
            results.forEach(result => createGenerator(result)); // מציג את כל הכרטיסיות שנמצאו
            search.innerHTML = "ניקוי"; // משנה את הטקסט של הכפתור ל"ניקוי"
            isClearing = true; // משנה את המצב של הכפתור ל"ניקוי"
        } else { // אם לא נמצאו תוצאות
            alert("לא נמצא ערך תואם."); // מציג הודעה אם לא נמצאו תוצאות
            searchInput.value = ""; // מנקה את שדה החיפוש אחרי ההודעה
            createAllCards(); // מציג מחדש את כל הכרטיסים
        }
    });
});
